...
public class HU_PersonNameParserCommandTest extends AbstractTest {
    ...
    @Test(dataProvider = "test_HU_1")
    public void test_HU_1(NaturalInputPerson inputPerson) throws Exception {
        PersonNameParserCommand command = new PersonNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        PersonNameParserResult result = executor.execute(command, mode, inputPerson).get();
        ParsedPerson parsedPerson = result.getBestMatch().getParsedPerson();
        OutputPersonName personName = parsedPerson.getOutputPersonName();
        assertEquals(personName.getFirst(TermType.GIVENNAME).get().getString(), "Levente");
        assertEquals(personName.getSecond(TermType.GIVENNAME).get().getString(), "Máté");
        assertEquals(personName.getFirst(TermType.SURNAME).get().getString(), "Szabó");
        assertEquals(parsedPerson.getGender().getGender(), ComputedPersonGender.MALE);
    }
    @DataProvider
    protected Object[][] test_HU_1() {
        return new Object[][]{
                {new NaturalInputPersonBuilder().name(new WesternInputPersonNameBuilder().fullname("Szabó Levente Máté").build()).build()},
